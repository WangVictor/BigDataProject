...
"BRONX":[40,41,42,44,46,48,52,50,43,49,45,47],"QUEENS":[114,115,108,110,104,112,102,109,107,106,113,111,105,103,100,101],"STATEN ISLAND":[120,122,123,121]}
    ...
    output13 = v1314.map(lambda x : ("NaN"+" TEXT location NULL") if len(x[0])==0 else (str(x[0])+" TEXT location VALID") if (x[0] in borough and int(x[1]) in dict[x[0]]) else (str(x[0])+" TEXT location VALID") if (x[0] in borough and len(x[1])==0) else (str(x[0])+ "TEXT category INVALID"))     
    output14 = v1314.map(lambda x : ("NaN"+" FLOAT location NULL") if len(x[1])==0 else (str(x[1])+" FLOAT location VALID") if (int(x[1]) in precinct and int(x[1]) in dict[x[0]]) else (str(x[1])+" FLOAT location VALID") if (int(x[1]) in precinct and len(x[0])==0) else (str(x[1])+ "FlOAT location INVALID"))
