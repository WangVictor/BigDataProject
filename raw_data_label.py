...
if __name__ == "__main__":
	...
	output2 = v2.map(lambda x : (str(x)+" DATETIME time VALID") if len(x)>0 else ("NaN"+" DATETIME time NULL"))
	v4 = lines.map(lambda x: x[4])
	output4 = v4.map(lambda x : (str(x)+" DATETIME time VALID") if len(x)>0 else ("NaN"+" DATETIME time NULL"))
